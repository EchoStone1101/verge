//! Tests for unit comparison APIs.

use core::cmp::Ordering;

use vstd::prelude::*;
use vstd::std_specs::cmp::{OrdSpec, PartialOrdSpec};
use verge::cmp::*;
use verge::prelude::*;

verus! {

fn test_unit_comparison_methods_are_callable() {
    proof {
        broadcast use verge::cmp::unit::group_unit_ordering;
    }

    let a = ();
    let b = ();

    let eq = a == b;
    let ne = a != b;
    let partial = a.partial_cmp(&b);
    let lt = a < b;
    let le = a <= b;
    let gt = a > b;
    let ge = a >= b;
    let cmp = a.cmp(&b);
    assert(eq);
    assert(!ne);
    assert(partial == Some(Ordering::Equal));
    assert(!lt);
    assert(le);
    assert(!gt);
    assert(ge);
    assert(cmp == Ordering::Equal);

    let method_eq = a.eq(&b);
    let method_ne = a.ne(&b);
    let method_lt = a.lt(&b);
    let method_le = a.le(&b);
    let method_gt = a.gt(&b);
    let method_ge = a.ge(&b);
    assert(method_eq);
    assert(!method_ne);
    assert(!method_lt);
    assert(method_le);
    assert(!method_gt);
    assert(method_ge);

    let max = a.max(b);
    let min = a.min(b);
    let clamp = a.clamp((), ());
    assert(max == ());
    assert(min == ());
    assert(clamp == ());
}

fn test_verified_bridge_lemmas_for_unit() {
    proof {
        broadcast use verge::cmp::unit::group_unit_ordering;
        lemma_partial_eq_verified::<()>();
        lemma_partial_ord_verified::<()>();
        lemma_ord_verified::<()>();
    }

    let a = ();
    let b = ();
    assert(<() as PartialOrdSpec>::partial_cmp_spec(&a, &b) == Some(Ordering::Equal));
    assert(<() as OrdSpec>::cmp_spec(&a, &b) == Ordering::Equal);
}

} // verus!
