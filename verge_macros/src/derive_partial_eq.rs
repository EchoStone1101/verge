...
pub fn derive_partial_eq_impl(attr: TokenStream, item: TokenStream) -> TokenStream {
    if !attr.is_empty() {
        return syn::Error::new_spanned(attr, "derive_partial_eq takes no arguments")
            .to_compile_error();
    }
    ...
    match item {
        ...
        _ => syn::Error::new(proc_macro2::Span::call_site(),
            "derive_partial_eq can only be applied to structs and enums").to_compile_error(),
    }
}
...
fn gen_struct(input: ItemStruct) -> TokenStream {
    ...
    emit_partial_eq(struct_def, name, &g, &tg, &openness, &code)
}
...
fn gen_enum(input: ItemEnum) -> TokenStream {
    ...
    let g = quote! { #generics };
    let tg = quote! { #ty_generics };
    emit_partial_eq(enum_def, name, &g, &tg, &openness, &code)
}
